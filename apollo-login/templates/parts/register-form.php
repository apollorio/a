<?php if ( ! defined( 'ABSPATH' ) ) { exit; } // plan-001 · G07 — file was directly reachable over HTTP ?>
<div class="apollo-register-form-wrapper">
	<form id="apollo-register-form" method="post">

		<!-- STEP 1: SOCIAL NAME (FIRST!) -->
		<div class="input-group apollo-step" data-step="1">
			...
			<input
				...
				required
				autocomplete="name"
				placeholder="<?php esc_attr_e( 'Seu nome social', 'apollo-login' ); ?>" class="apollo-input"
			/>
		...
		<div class="input-group apollo-step" data-step="2" style="display:none;">
			...
			<input
				...
				required
				autocomplete="off"
				placeholder="<?php esc_attr_e( 'seu_instagram', 'apollo-login' ); ?>" class="apollo-input"
			/>
		...
		<!-- STEP 3: EMAIL -->
		<div class="input-group apollo-step" data-step="3" style="display:none;">
			<label for="reg-email">
			...
			<input
				...
				required
				autocomplete="email"
				placeholder="<?php esc_attr_e( 'seu@email.com', 'apollo-login' ); ?>" class="apollo-input"
			/>
		</div>

		<!-- STEP 4: PASSWORD -->
		<div class="input-group apollo-step" data-step="4" style="display:none;">
			...
			<input
				...
				placeholder="<?php esc_attr_e( 'Mínimo 8 caracteres', 'apollo-login' ); ?>" class="apollo-input"
			/>
		</div>

		<!-- STEP 5: PASSWORD CONFIRM -->
		<div class="input-group apollo-step" data-step="5" style="display:none;">
			...
			<input
				...
				required
				autocomplete="new-password"
				placeholder="<?php esc_attr_e( 'Mesma senha de novo', 'apollo-login' ); ?>" class="apollo-input"
			/>
		</div>

		<!-- STEP 6: SOUND SELECTION - GLOBAL BRIDGE TAXONOMY -->
		<div class="input-group apollo-sound-selection apollo-step" data-step="6" style="display:none;">
			<label>
			...
			<div class="apollo-sound-grid" id="apollo-sound-grid">
				...
				if ( ! empty( $sounds ) && ! is_wp_error( $sounds ) ) :
					foreach ( $sounds as $sound ) :
						...
						?>
						<label class="custom-checkbox" class="apollo-sound-chip" data-sound-id="<?php echo esc_attr( $sound->term_id ); ?>">
							<input
								type="checkbox"
								name="sounds[]"
			...
			success: function(response) {
				if (response.success) {
					$message.removeClass('error').addClass('success')
						...
						.show();
					setTimeout(function() {
						window.location.href = '<?php echo esc_url( \Apollo\Login\apollo_login_canonical_login_url() ); ?>';
					}, 2000);
				}
			},
			error: function(xhr) {
