...
?>
    ...
    <body style="display:flex;align-items:center;justify-content:center;min-height:100vh;min-height:100dvh;font-family:'Space Grotesk',sans-serif;">
        <div style="text-align:center">
            <h1 style="font-size:3rem;font-weight:800;">404</h1>
            <p style="color:#94a3b8;">Comuna ou núcleo não encontrado.</p>
            <a href="<?php echo esc_url(home_url('/grupos')); ?>" style="color:FF9820;font-weight:600;">← Voltar para grupos</a>
        </div>
    </body>
...
// ── Privacy Gate — private/secret groups require membership ──
$group_privacy = $group['privacy'] ?? 'public';
if (
    'public' !== $group_privacy
    && ! current_user_can('manage_options')
    && ( ! $is_logged || ! function_exists('apollo_is_group_member') || ! apollo_is_group_member((int) ($group['id'] ?? 0), $user_id) )
) {
    status_header(403);
?>
    <!DOCTYPE html>
    ...
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width,initial-scale=1">
        <title>Apollo · Acesso Restrito</title>
        <script src="<?php echo esc_url( function_exists('apollo_cdn_core_js_url') ? apollo_cdn_core_js_url() : 'https://cdn.apollo.rio.br/v1.0.0/core.js?v=t0x1x&versao=bb' ); ?>" fetchpriority="high" crossorigin="anonymous"></script>
    </head>

    <body style="display:flex;align-items:center;justify-content:center;min-height:100vh;min-height:100dvh;font-family:'Space Grotesk',sans-serif;">
        <div style="text-align:center">
            <i class="ri-lock-fill" style="font-size:3rem;color:FF9820;"></i>
            <h1 style="font-size:1.75rem;font-weight:800;margin-top:12px;">Acesso Restrito</h1>
            <p style="color:#94a3b8;max-width:340px;margin:8px auto 20px;">Este grupo é privado. Apenas membros podem visualizar o conteúdo.</p>
            <a href="<?php echo esc_url(home_url('/grupos')); ?>" style="color:FF9820;font-weight:600;">← Voltar para grupos</a>
        </div>
    </body>
