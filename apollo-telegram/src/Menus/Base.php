...
abstract class Base
{
	protected $optionsName = APOLLO_TELEGRAM_MENUS_SLUG . '_options';

	/**
	 * **(REQUIRED)** Slug for this submenu. Recommended format: `APOLLO_TELEGRAM_MENUS_SLUG . '_{NAME}'`.
	 *
	 * @var	string
	 */
	protected $menuSlug = '';
	...
	public function __construct()
	{
		...
		add_filter('APOLLO_TELEGRAM_menus_submenus', [$this, 'addSubmenu']);
	...
	public function validateSettings($input)
	{
		...
		$input = apply_filters('APOLLO_TELEGRAM_before_validate_settings', $input, $options);

		if (is_array($input))
		{
			// Allow values modification via filters
			foreach ($input as $key => $value)
				$input[$key] = apply_filters("APOLLO_TELEGRAM_validate_input_$key", $value, $key, $input, $options);
		...
		do_action('APOLLO_TELEGRAM_validate_settings', $input, $options);
	...
	public function inputFieldCallback($args)
	{
		...
		APOLLO_TELEGRAM()->view('admin.settings.fields.input', $this->getSettingsValue($id, $args));
	}
	...
	public function textareaFieldCallback($args)
	{
		...
		APOLLO_TELEGRAM()->view('admin.settings.fields.textarea', $this->getSettingsValue($id, $args));
	}
	...
	public function selectFieldCallback($args)
	{
		...
		APOLLO_TELEGRAM()->view('admin.settings.fields.select', $this->getSettingsValue($id, $args));
	}
	...
	private function getSettingsValue($key, $args)
	{
		...
		$value = isset($args['value']) ? $args['value'] : APOLLO_TELEGRAM()->option($key);
	...
	public function displayContent()
	{
		...
		$args       = [
			'title'     => __('Settings', 'apollo-telegram'),
			'menuSlug'  => $this->menuSlug,
			'tabs'      => $this->getTabs(),
			'activeTab' => $activeTab,
		];

		APOLLO_TELEGRAM()->view('admin.settings.wrapper', $args);
	}
}
