...
class Menu
{
	...
	private $menuSlug = APOLLO_TELEGRAM_MENUS_SLUG . '_settings';
	...
	public function __construct(Container $container)
	{
		...
		add_filter('plugin_action_links_' . APOLLO_TELEGRAM_BASENAME, [$this, 'actionLinks']);
	}
	...
	{
		foreach (glob(APOLLO_TELEGRAM_DIR . '/src/Menus/*Menu.php') as $file)
		{
	...
	{
		add_menu_page(
			esc_html__('Telegram Bot Boilerplate Plugin', 'apollo-telegram'),
			esc_html__('Telegram Bot', 'apollo-telegram'),
			'manage_options',
			$this->menuSlug,
			'',
			'dashicons-heart'
		...
		$position = 0;
		foreach (apply_filters('APOLLO_TELEGRAM_menus_submenus', []) as $slug => $submenu)
		{
			...
			add_submenu_page(
				...
				APOLLO_TELEGRAM_MENUS_SLUG . '_' . sanitize_key($slug),
				$submenu['callback'],
	 ...
	 * @return	array
	 *
	 * @hooked	filter: `plugin_action_links_{APOLLO_TELEGRAM_BASENAME}` - 10
	 */
	public function actionLinks($links)
	{
		$links[] = '<a href="' . get_admin_url(null, "admin.php?page={$this->menuSlug}") . '">' . esc_html__('Settings', 'apollo-telegram') . '</a>';
		return $links;
	}
}
