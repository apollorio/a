...
class Model
{
	...
	{
		if (trim(get_option(APOLLO_TELEGRAM_OPTIONS_KEY_DB_VERSION, '')) === trim(APOLLO_TELEGRAM_DB_VERSION)) {
			return;
		}
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_user` (
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_chat` (
				...
				PRIMARY KEY (`id`, `bot_username`),
				KEY `old_id` (`old_id`)
			) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_520_ci;"
		);

		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_user_chat` (
				`user_id` BIGINT COMMENT 'Unique user identifier',
				`chat_id` BIGINT COMMENT 'Unique user or chat identifier',
				`bot_username` CHAR(191) NOT NULL COMMENT 'Current bot''s username',

				PRIMARY KEY (`user_id`, `chat_id`, `bot_username`)#,
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_inline_query` (
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_chosen_inline_result` (
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_message` (
				...
				`forum_topic_created` TEXT NULL DEFAULT NULL COMMENT 'Service message: forum topic created',
				`forum_topic_edited` TEXT DEFAULT NULL COMMENT 'Service message: forum topic edited',
				`forum_topic_closed` TEXT NULL DEFAULT NULL COMMENT 'Service message: forum topic closed',
				`forum_topic_reopened` TEXT NULL DEFAULT NULL COMMENT 'Service message: forum topic reopened',
				...
				PRIMARY KEY (`chat_id`, `id`, `bot_username`),
				KEY `user_id` (`user_id`),
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_bot_message` (
				`chat_id` BIGINT COMMENT 'Unique chat identifier',
				`sender_chat_id` BIGINT COMMENT 'Sender of the message, sent on behalf of a chat',
				`id` BIGINT UNSIGNED COMMENT 'Unique message identifier',
				`bot_username` CHAR(191) NOT NULL COMMENT 'Current bot''s username',
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_edited_message` (
				`id` BIGINT UNSIGNED AUTO_INCREMENT COMMENT 'Unique identifier for this entry',
				`chat_id` BIGINT COMMENT 'Unique chat identifier',
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_callback_query` (
				`id` BIGINT UNSIGNED COMMENT 'Unique identifier for this query',
				`user_id` BIGINT NULL COMMENT 'Unique user identifier',
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_shipping_query` (
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_pre_checkout_query` (
				`id` BIGINT UNSIGNED COMMENT 'Unique query identifier',
				`user_id` BIGINT COMMENT 'User who sent the query',
				`bot_username` CHAR(191) DEFAULT NULL COMMENT 'Current bot''s username',
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_poll` (
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_poll_answer` (
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_chat_member_updated` (
				`id` BIGINT UNSIGNED AUTO_INCREMENT COMMENT 'Unique identifier for this entry',
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_chat_join_request` (
				`id` BIGINT UNSIGNED AUTO_INCREMENT COMMENT 'Unique identifier for this entry',
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_telegram_update` (
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_conversation` (
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_request_limiter` (
				`id` BIGINT UNSIGNED AUTO_INCREMENT COMMENT 'Unique identifier for this entry',
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_chat_list` (
				`id` BIGINT UNSIGNED AUTO_INCREMENT COMMENT 'Unique identifier for this entry',
		...
		$this->wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$this->wpdb->prefix}APOLLO_TELEGRAM_chat_messages` (
				`id` BIGINT UNSIGNED AUTO_INCREMENT COMMENT 'Unique identifier for this entry',
				`chat_list_id` longtext NOT NULL,
				`telegram_message_id` longtext NOT NULL,
				`reply_to_id` longtext NOT NULL,
				`telegram_id` longtext NOT NULL,
				`bot_username` longtext NOT NULL,
				`type` text NOT NULL,
				`status` text NOT NULL,
				`forward_to` longtext NOT NULL,
		...
		update_option(APOLLO_TELEGRAM_OPTIONS_KEY_DB_VERSION, trim(APOLLO_TELEGRAM_DB_VERSION));
	}
}
