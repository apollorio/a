<?php
/**
 * Resale ticket — /anuncios carousel card (mockup top/bottom contract).
 *
...
$cta_label = $locked
	? __( 'Entre para falar', 'apollo-adverts' )
	: __( 'Falar', 'apollo-adverts' );
if ( $sold ) {
	$cta_label = __( 'Encerrado', 'apollo-adverts' );
}
...
$classes = array( 'rt-card', 'carousel-item' );
if ( $locked ) {
	$classes[] = 'is-guest';
}
if ( $sold ) {
...
>
	<div class="top" role="button" tabindex="<?php echo ! empty( $d['aria_hidden'] ) ? '-1' : '0'; ?>">
		<div class="rt-media">
			...
			<div class="rt-mediafoot">
				...
				<?php endif; ?>
				<h3><?php echo $locked ? esc_html__( 'Ingresso', 'apollo-adverts' ) : esc_html( $title ); ?></h3>
			</div>
		</div>

		<div class="rt-summary">
			<div class="rt-chips">
				<span class="rt-chip qty"><?php echo esc_html( $qty . ' UND' ); ?></span>
				<?php if ( $type_lbl ) : ?>
			...
			<div class="rt-price">
				...
				<?php elseif ( $has_price ) : ?>
					<?php echo esc_html( $fmt( $total ) ); ?>
					<small><?php echo esc_html( $fmt( $each ) . ' cada' ); ?></small>
				<?php else : ?>
	...
	<div class="bottom">
		<div class="rt-barcode-row">
			<div class="barcode"><?php echo $bars; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
		...
		<?php else : ?>
			...
			>
				<i class="<?php echo $locked ? 'ri-lock-2-line' : 'ri-chat-3-line'; ?>" aria-hidden="true"></i>
